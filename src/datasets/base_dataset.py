...
class BaseDataset(Dataset, ABC):
    ...
    def __getitem__(self, ind):
        ...
        lensed, lensless, psf = get_dataset_object(
            lensed=data_dict["lensed"],
            lensless=data_dict["lensless"],
            mask_vals=self.get_mask(data_dict["mask_label"])
        )
        instance_data = {"lensed": lensed, "lensless": lensless, "psf": psf}
    ...
    def _assert_index_is_valid(index):
        ...
        Args:
            index (Dataset): HuggingFace Dataset, containing dict for each element of
                ...
                such as label and image.
        """
        required_columns = {"lensless", "lensed", "mask_label"}
        actual_columns = set(index.column_names)
